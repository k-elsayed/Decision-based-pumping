...
class benchmark:
    ...
    def __init__(self,sample_size=1000, R1_arr_rate=0.4, R2_arr_rate=0.8, delta_t_g=0.1, T=10,
                 F_E_0=0.7, F_R_1=0.5, F_R_2=0.5, eta_db=0.15, l_km=1, p_g=0.8, t_c=1,
                 ):
    ...
    def next_event(self,skip_ent):
        ...
        if time_next_event == time_next_req:
            if(self.request_sequence[self.req_index+1]=='R_1'):
                self.F_r= self.F_R_1
            else:
                self.F_r = self.F_R_2
            self.flag=1
...
run_param_path = '/home/karim/My Work/Codes/Decision based pumping with Anam/RL4QEntanglements/Results/p/TwoPoisson/run_params_2Pois.json'
...
R1_arr_rate = request_arrival_rates[0]
R2_arr_rate = request_arrival_rates[1]
...
Benchmarking=benchmark(sample_size, R1_arr_rate, R2_arr_rate, delta_t_g, T,
                 F_E_0, F_R_1, F_R_2, eta_db, l_km, p_g, t_c)
